...
- **2026-09-19, merged upstream `4b1cbedf39a` (563 commits) into `self-hosted`.** Done as a merge, not a rebase: `self-hosted` already contains merge commits from an earlier GitHub-side "sync fork", so a rebase would replay them. No conflicts. Reviewed the incoming diff for new gates: only the two frontend AI-subscription gates needed a patch (row 9). Checked and left alone: `_toolbar_entitlements` (already unlocked for non-Cloud), `is_team_over_ai_credit_budget` (reads a quota cache that self-hosted never fills), `LOGS_RETENTION_30D` gating (covered by the all-features-unlocked patch #1), and the new `posthog.llm.gateway_client` (only calls the URL configured in `LLM_GATEWAY_URL`, so nothing goes to a PostHog-owned host).
